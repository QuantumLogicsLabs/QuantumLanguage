...
// True if the block opened at `openerIdx` uses `break` or `next` directly in
// its own body (not inside a nested block, which owns its own loop control).
//
// This decides how the block is emitted. Ruby's `break`/`next` are *loop*
// control, but a block normally becomes a closure argument — and neither
// keyword can cross a function boundary, so `arr.each do |x| ... break ... end`
// compiled as a closure is at best wrong and at worst a stack-overflow crash.
// When they are present the block is emitted as a native `for ... in` loop
// instead, where both keywords mean exactly what Ruby intends.
static bool rbBlockBodyUsesLoopControl(const std::vector<std::string> &lines, size_t openerIdx)
{
    int depth = 1;
    for (size_t i = openerIdx + 1; i < lines.size(); i++)
    {
        ...
        if (depth != 1)
            continue; // belongs to a nested block, not this one
        if (t == "break" || t == "next" ||
            startsWith(t, "break ") || startsWith(t, "next ") ||
            startsWith(t, "break(") || startsWith(t, "next("))
            return true;
    }
    return false;
...
enum class RBFrameKind { Other, Def, ClosureDo, ClosureLambda, Branch, Loop, Class };
...
std::string applyRubyDialect(const std::string &source, bool strict)
{
        ...
        [&](const std::string &codeIn) -> std::string
    {
        ...
        if (strict)
        {
            ...
            if (rbTryMultiAssign(code, expanded))
            {
                ...
                for (auto &e : expanded)
                {
                    if (!joined.empty())
                        joined += "; ";
                    joined += rbNormalizeAtoms(rbConvertRanges(e, strict), symbols, strict, inClassBody);
                }
                return joined;
            }
        }
        ...
        // not just at true statement/line level.
        {
            ...
            static const std::regex modIfRe("^(.*\\S)\\s+(if|unless|while|until)\\s+(.+)$");
            std::smatch m;
            if (!startsWith(code, "if ") && !startsWith(code, "unless ") &&
                !startsWith(code, "while ") && !startsWith(code, "until ") &&
                ...
                trimCopy(m[1].str()).back() != '=')
            {
                ...
                std::string cond = rbSubstituteMainGuard(rbNormalizeAtoms(rbConvertInterpolation(rbConvertRanges(trimCopy(m[3].str()), strict), symbols), symbols, strict, inClassBody));
        ...
        {
            std::string prefix = rbNormalizeAtoms(rbConvertRanges(block.prefix, strict), symbols, strict, inClassBody);
        ...
        // must be left alone.
        if (strict && !code.empty() && code[0] == '[' && previousLineCanContinueExpression())
        {
    ...
    for (size_t li = 0; li < rawLines.size(); li++)
    {
            ...
            stack.back().kind == RBFrameKind::Branch)
        {
            ...
            stack.pop_back();
            std::string cond = rbNormalizeAtoms(rbConvertInterpolation(rbConvertRanges(trimCopy(code.substr(6)), strict), symbols), symbols, strict, inClassBody);
        ...
        if (strict)
        {
            ...
            {
                std::string target = rbNormalizeAtoms(rbConvertRanges(trimCopy(m[1].str()), strict), symbols, strict, inClassBody);
                std::string cond = rbNormalizeAtoms(rbConvertInterpolation(rbConvertRanges(trimCopy(m[3].str()), strict), symbols), symbols, strict, inClassBody);
                if (m[2].str() == "unless")
                    cond = "!(" + cond + ")";
        ...
        {
            std::string cond = rbSubstituteMainGuard(rbNormalizeAtoms(rbConvertInterpolation(rbConvertRanges(trimCopy(code.substr(7)), strict), symbols), symbols, strict, inClassBody));
            outLines.push_back(indentation + "if (!(" + cond + ")) {");
        ...
        {
            std::string cond = rbSubstituteMainGuard(rbNormalizeAtoms(rbConvertInterpolation(rbConvertRanges(trimCopy(code.substr(3)), strict), symbols), symbols, strict, inClassBody));
            outLines.push_back(indentation + "if (" + cond + ") {");
            stack.push_back(RBFrame{RBFrameKind::Branch, nextChainGroup++, RBTailInfo{}});
            continue;
        }
        ...
        {
            std::string subject = rbNormalizeAtoms(rbConvertInterpolation(rbConvertRanges(trimCopy(code.substr(5)), strict), symbols), symbols, strict, inClassBody);
        ...
        }
        {
                ...
                (stack.back().chainGroupId == -2 || stack.back().kind == RBFrameKind::Branch))
            {
                ...
                std::string valuesText = rbNormalizeAtoms(rbConvertInterpolation(rbConvertRanges(m[1].str(), strict), symbols), symbols, strict, inClassBody);
            ...
            (strict || (rbUnambiguous(code) && rbHasMatchingEnd(rawLines, li))))
        {
            ...
            if (strict)
            {
                ...
                if (std::regex_match(prefix, lm, lambdaDoRe))
                {
                    ...
                    stack.push_back(RBFrame{RBFrameKind::ClosureLambda, -1, RBTailInfo{}});
                    continue;
                }
            }
            std::string convertedPrefix = rbNormalizeAtoms(rbConvertRanges(prefix, strict), symbols, strict, inClassBody);
            ...
            if (strict && rbBlockBodyUsesLoopControl(rawLines, li))
            {
                ...
                std::smatch em;
                auto names = rbSplitTopLevel(params, ',');
                if (std::regex_match(convertedPrefix, em, eachCallRe) && names.size() == 1 &&
                    !trimCopy(names[0]).empty())
                {
            ...
            if (strict && !convertedPrefix.empty() && convertedPrefix[0] == '[' && previousLineCanContinueExpression())
            {
