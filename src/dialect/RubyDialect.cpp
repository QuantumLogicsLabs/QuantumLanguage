...
// Quantum keywords (src/lexer/LexerCore.cpp) that are ordinary identifiers
// in Ruby. A Ruby program is free to name a variable, parameter or method
// `from`/`of`/`in`/`hash`/..., which the Quantum lexer would tokenize as a
// keyword and fail to parse — so those names are consistently renamed.
// Deliberately excluded: names Ruby itself treats as keywords or that the
// transpiler maps on purpose (`self`, `class`, `def`, `if`, `nil`, `print`,
// `input` — the last has its own narrower rule just below).
static const std::set<std::string> &rbQuantumKeywords() {
  // Deliberately NOT listed: `fn`, `let`, `function`, `return`, `self` and
  // friends — this pass runs over text the transpiler has already emitted
  // (`fn(x) {`, `let tmp = ...`), so escaping them would corrupt its own
  // output. Only names Ruby code realistically uses as identifiers, and
  // that Quantum's lexer would otherwise tokenize as keywords, belong here.
  static const std::set<std::string> names = {
      "from",    "of",      "is",   "as",   "extends", "scan", "payload",
      "encrypt", "decrypt", "hash", "cout", "cin",     "endl"};
  return names;
}

static const std::set<std::string> &rbZeroArgMethods(bool strict) {
  static const std::set<std::string> strictNames = {
      "reverse", "chomp", "downcase", "upcase", "dup", "clone", "sort", "strip",
      "trim", "to_i", "to_f", "to_s", "zero", "chars", "length", "size", "now",
      "message",
      // Collection/string queries idiomatically called without parens.
      "shift", "pop", "empty", "keys", "first", "last", "sum", "uniq",
...
struct RubySymbolTable {
  ...
  std::set<std::string>
      blockMethods; // methods whose body uses yield/block_given?
};
...
static std::string
rbNormalizeAtoms(const std::string &s,
                 const RubySymbolTable &st = rbEmptySymbolTable(),
                 bool strict = true, bool inClassBody = false) {
  ...
  for (size_t i = 0; i < s.size();) {
    ...
    if (std::isdigit((unsigned char)c)) {
      size_t j = i;
      while (j < s.size() &&
             (std::isdigit((unsigned char)s[j]) || s[j] == '_' ||
              (s[j] == '.' && j + 1 < s.size() &&
    ...
    if (rbIsIdentStart(c)) {
      ...
      if (ident == "input" && !followedByParen)
        ident = "__rb_input";
      // Ordinary Ruby identifiers that happen to be Quantum keywords
      // (`def add_edge(from, to, weight)` — `from` is Quantum's
      // import-from keyword, so the parameter list fails to parse).
      // Renaming every occurrence consistently keeps the program's
      // meaning while getting it past the lexer. Strict (.rb) only:
      // in a mixed .sa file these words may genuinely be keywords.
      else if (strict && !hadDotBefore && rbQuantumKeywords().count(ident))
        ident = "__rb_" + ident;
      // A receiver-less `name?`/`name!` is *always* a method call in
      // Ruby — a local variable's name cannot contain `?` or `!` — so
      // inside a class body it is an implicit-self call and needs both
      // the receiver and the call parens: `full?` -> `self.full()`.
      // (The paren-ed form `prime?(n)` is handled by selfCallRe, and a
      // top-level `def` is a plain function with no self to qualify.)
      if (strict && inClassBody && suffixStripped && !hadDotBefore &&
          !followedByParen && st.methods.count(ident)) {
        out += "self." + ident + "()";
        i = j;
        continue;
      }
...
static bool rbBlockBodyUsesLoopControl(const std::vector<std::string> &lines,
                                       size_t openerIdx) {
  ...
  for (size_t i = openerIdx + 1; i < lines.size(); i++) {
    ...
    if (rbLineIsRubyOpener(t)) {
      bool isLoopLike =
          startsWith(t, "while ") || startsWith(t, "until ") ||
          startsWith(t, "for ") || t == "loop" || t == "loop do" ||
          startsWith(t, "def ") || startsWith(t, "class ") ||
          startsWith(t, "module ") ||
          std::regex_search(t, std::regex("\\bdo(\\s*\\|[^|]*\\|)?\\s*$"));
      loopStack.push_back(isLoopLike);
      continue;
    }
    if (t == "break" || t == "next" || startsWith(t, "break ") ||
        startsWith(t, "next ") || startsWith(t, "break(") ||
        startsWith(t, "next(")) {
      bool insideNestedLoop = false;
      for (bool isLoop : loopStack) {
        if (isLoop) {
...
static void rbCollectSymbols(const std::vector<std::string> &lines,
                             RubySymbolTable &st) {
  ...
  static const std::regex explicitBlockParamRe(
      "[(,]\\s*&[A-Za-z_][A-Za-z0-9_]*\\s*\\)");
  std::string
      currentMethod; // name of the def whose body we're currently inside
  for (auto &line : lines) {
...
static std::string rbQualifySelf(const std::string &expr,
                                 const RubySymbolTable &st) {
  static const std::regex classNameRe(
      "\\b([A-Za-z_][A-Za-z0-9_]*)\\.class\\.name\\b");
...
static std::string rbConvertInterpolation(const std::string &line,
                                          const RubySymbolTable &st) {
  std::string out;
  size_t i = 0;
  while (i < line.size()) {
    ...
    if (c == '"') {
      ...
      while (p < inner.size()) {
        ...
        rebuilt += "${" +
                   rbQualifySelf(rbNormalizeAtoms(trimCopy(exprText), st), st) +
                   "}";
...
static std::string rbConvertRanges(std::string line, bool strict = true) {
  ...
  static const std::regex arrayNewFillRe(
      "Array\\.new\\(([^(),]*),\\s*([^()]*)\\)");
  line = std::regex_replace(line, arrayNewFillRe,
                            "range(0, $1).map(fn(__ai) { return $2 })");
  static const std::regex arrayNewSizeRe("Array\\.new\\(([^(),]*)\\)");
  line = std::regex_replace(line, arrayNewSizeRe,
                            "range(0, $1).map(fn(__ai) { return nil })");
  // Ruby's `ClassName.new(args)` constructor call -> Quantum's
  // `ClassName(args)`. The builtin modules below are provided as native objects
  // that expose their own `new`, so their `.new` must survive this rewrite.
  ...
  static const std::regex newCallRe(std::string("\\b") + kBuiltinModules +
                                    "([A-Za-z_][A-Za-z0-9_]*)\\.new\\(");
  ...
  static const std::regex newBareRe(std::string("\\b") + kBuiltinModules +
                                    "([A-Za-z_][A-Za-z0-9_]*)\\.new\\b(?!\\()");
  ...
  static const std::regex builtinNewBareRe(
      "\\b(Thread|Mutex|ConditionVariable|Queue|SizedQueue|Ractor|Fiber|Set)\\."
      "new\\b(?!\\()");
  ...
  static const std::regex spaceshipRe("([A-Za-z_][A-Za-z0-9_.\\[\\]]*)\\s*<=>"
  ...
  // The receiver is either a name/member/call chain or a parenthesized
  // expression — `(s + "$")[idx..-1]` is as common as `s[idx..-1]`.
  static const std::regex sliceRangeRe(
      "(\\([^()]*\\)|(?:@|self\\.)?[A-Za-z_][A-Za-z0-9_]*(?:\\.[A-Za-z_][A-Za-"
      "z0-9_]*)*"
      "(?:\\([^()]*\\))?)\\[([^\\[\\]]+?)(\\.\\.\\.?)([^\\[\\]]*?)\\]");
  {
    std::smatch m;
    while (std::regex_search(line, m, sliceRangeRe)) {
      ...
      line = line.substr(0, m.position()) + "__slice_range(" + m[1].str() +
             ", " + trimCopy(m[2].str()) + ", " + upper + ", " + inclusive +
             ")" + line.substr(m.position() + m.length());
    }
  }
...
static std::string rbConvertBlockCapture(std::string line, bool strict) {
  ...
  static const std::regex blockCallBareRe(
      "\\b([A-Za-z_][A-Za-z0-9_]*)\\.call\\b(?!\\()");
...
// Ruby's `raise ClassName, "message"` comma form -> `raise
// ClassName("message")`.
...
static std::string rbSubstituteMainGuard(const std::string &cond) {
  static const std::regex mainGuardRe("__FILE__\\s*==\\s*\\$(0|PROGRAM_NAME)|"
                                      "\\$(0|PROGRAM_NAME)\\s*==\\s*__FILE__");
...
static bool rbPrefixEndsWithBlockMethod(const std::string &prefix,
  ...
  // (`obj.traverse(x) { }`).
  static const std::regex callTailRe(
      "(?:^|\\.)([A-Za-z_][A-Za-z0-9_]*)[?!]?(\\([^()]*\\))?$");
  std::smatch m;
  if (std::regex_search(prefix, m, callTailRe) &&
      st.blockMethods.count(m[1].str()))
    return true;
  return false;
}
...
static bool rbTryInlineBraceBlock(const std::string &code, RubyBlockCall &out,
  ...
  if (!code.empty() && code[0] == '}')
    return false; // continuation of an earlier brace construct, not a fresh
                  // block
...
static bool rbTryTrailingDoOpener(const std::string &code, std::string &prefix,
                                  std::string &params) {
  static const std::regex doRe("^(.*?)\\bdo(?:\\s*\\|([^|]*)\\|)?\\s*$");
  std::smatch m;
...
static std::string rbApplyBlockDestructuring(const std::string &prefix,
                                             std::string &params) {
  ...
  for (size_t k = 0; k < names.size(); k++)
    bodyPrefix += "let " + trimCopy(names[k]) + " = " + tmp + "[" +
                  std::to_string(k) + "]; ";
  params = tmp;
  return bodyPrefix;
}
...
static std::string rbBuildBlockOpenText(const std::string &prefix,
...
struct RBFrame {
  ...
  bool usesImplicitBlock =
      false; // Def only: body referenced yield/block_given?
...
static std::vector<std::string>
rbFlattenModules(std::vector<std::string> lines) {
  static const std::regex moduleRe("^module\\s+([A-Za-z_][A-Za-z0-9_]*)\\s*$");
  static const std::regex includeRe(
      "^include\\s+([A-Za-z_][A-Za-z0-9_]*)\\s*$");

  auto isOpener = [](const std::string &t) -> bool {
    if (startsWith(t, "module ") || startsWith(t, "class ") ||
        startsWith(t, "def ") || t == "begin" || startsWith(t, "if ") ||
        startsWith(t, "unless ") || startsWith(t, "while ") ||
        startsWith(t, "until ") || startsWith(t, "case "))
      return true;
    static const std::regex doRe("\\bdo(\\s*\\|[^|]*\\|)?\\s*$");
  ...
  // `include Enumerable` splices in the synthesized mixin (RubyStdlib.cpp),
  // through the same path as a user-defined `module ... end` below.
  modules["Enumerable"] = rubyEnumerableModuleSource();
...
static std::string rbMatchDefaultHashDecl(const std::string &code,
                                          RubyDefaultHash &out) {
  static const std::regex blockFormRe(
      "^(@?[A-Za-z_][A-Za-z0-9_]*)\\s*=\\s*Hash\\.new\\s*\\{\\s*\\|[^|]*\\|\\s*"
      "[A-Za-z_][A-Za-z0-9_]*\\[[^\\]]*\\]\\s*=\\s*(.+?)\\s*\\}\\s*$");
  static const std::regex valueFormRe(
      "^(@?[A-Za-z_][A-Za-z0-9_]*)\\s*=\\s*Hash\\.new\\((.*)\\)\\s*$");
  static const std::regex emptyFormRe(
      "^(@?[A-Za-z_][A-Za-z0-9_]*)\\s*=\\s*Hash\\.new\\s*$");
  std::smatch m;
...
static std::string
rbApplyDefaultHashReads(std::string code,
  ...
  for (auto &entry : hashes) {
    ...
    code = std::regex_replace(code, readRe,
                              helper + "(" + name + ", $1, " + def + ")");
  }
  return code;
}
...
static std::string rbLocalDeclName(const std::string &code,
                                   std::set<std::string> &declared) {
  static const std::regex simpleAssignRe(
  ...
  // Also parse chained assignments: slow = fast = expr -> record fast as declared too
  std::string rhs = m[2].str();
  std::smatch mRhs;
  while (std::regex_match(rhs, mRhs, simpleAssignRe)) {
    static const std::set<std::string> res = {
        "self", "this", "true", "false", "nil", "null", "let", "const", "return"};
    std::string subName = mRhs[1].str();
    if (!res.count(subName))
      declared.insert(subName);
    rhs = mRhs[2].str();
  }
...
static bool rbTryMultiAssign(const std::string &code,
                             std::vector<std::string> &outLines) {
  size_t eq = rbFindTopLevelAssign(code);
  if (eq == std::string::npos)
    return false;
  // A trailing statement modifier belongs to the whole assignment
  // (`l, r = i, i + 1 if cond`) and must be peeled off by the modifier
  // pass first — expanding here would glue the condition onto the last
  // value and silently change the program's meaning.
  for (const char *kw : {" if ", " unless ", " while ", " until "})
    if (rbFindTopLevel(code, kw) != std::string::npos)
      return false;
  ...
  if (targets.size() < 2 || targets.size() != values.size())
    return false;
  static const std::regex targetRe(
      "^[A-Za-z_][A-Za-z0-9_]*(\\[[^\\]]*\\]|\\.[A-Za-z_][A-Za-z0-9_]*)*$");
  for (auto &t : targets)
...
std::string applyRubyDialect(const std::string &source, bool strict) {
  ...
  std::map<size_t, std::string>
      caseSubjects; // stack-depth -> subject, before first `when`
  std::map<int, std::string>
      caseSubjectByGroup; // chainGroupId -> subject, after first `when`
  ...
  auto emit = [&](const std::string &indentation, const std::string &code,
                  bool trackAsStatement) {
  ...
  // Whether the last thing emitted could have an expression continue onto
  // the next line. It cannot after a `{`, `}` or `;` — and that is exactly
  // when the array-literal `;` disambiguation guard must be suppressed,
  // since `{ ;[...]` is an empty statement the parser rejects.
  auto previousLineCanContinueExpression = [&]() -> bool {
    for (auto it = outLines.rbegin(); it != outLines.rend(); ++it) {
      std::string t = trimCopy(*it);
      if (t.empty() || startsWith(t, "//"))
        continue;
      char last = t.back();
      return last != '{' && last != '}' && last != ';';
    }
    return false; // nothing emitted yet — start of file
  };

  std::function<void(const RBTailInfo &)> resolveTail =
      [&](const RBTailInfo &info) {
  ...
  std::function<std::string(const std::string &)> transformCore =
    ...
    // us just the statement half. Expand it into a `;`-joined sequence
    // so it still fits inside the generated `if (...) { ... }`.
    if (strict) {
      ...
      if (rbTryMultiAssign(code, expanded)) {
        ...
        for (auto &e : expanded) {
          if (!joined.empty())
            joined += "; ";
          joined += rbNormalizeAtoms(rbConvertRanges(e, strict), symbols,
                                     strict, inClassBody);
        }
        return joined;
      }
    ...
    // Transform Ruby's safe navigation operator `&.` into a ternary
    // e.g. `obj&.method` -> `(obj != nil ? obj.method : nil)`
    if (strict) {
      size_t pos;
      while ((pos = code.find("&.")) != std::string::npos) {
        // Find the boundary of the receiver expression before `&.`
        // This is a simplistic backward scan matching typical receivers
        // (variable names or simple calls)
        int start = pos - 1;
        int parens = 0;
        int brackets = 0;
        while (start >= 0) {
        ...
        if (start < 0)
          start = 0;
        std::string receiver = trimCopy(code.substr(start, pos - start));

        // Find the method chain after `&.`
        int end = pos + 2;
        parens = 0;
        brackets = 0;
        while (end < (int)code.length()) {
          char c = code[end];
          if (c == '(')
            parens++;
          else if (c == ')')
        ...
        std::string methodChain = code.substr(pos + 2, end - pos - 2);

        std::string replacement = "(" + receiver + " != nil ? " + receiver +
                                  "." + methodChain + " : nil)";
        code = code.substr(0, start) + replacement + code.substr(end);
      }
    }
    ...
    {
      // Also covers Ruby's loop modifiers, `stmt while c` / `stmt until c`.
      static const std::regex modIfRe(
          "^(.*\\S)\\s+(if|unless|while|until)\\s+(.+)$");
      std::smatch m;
      if (!startsWith(code, "if ") && !startsWith(code, "unless ") &&
          !startsWith(code, "while ") && !startsWith(code, "until ") &&
        ...
        std::string cond = rbSubstituteMainGuard(rbNormalizeAtoms(
            rbConvertInterpolation(
                rbConvertRanges(trimCopy(m[3].str()), strict), symbols),
            symbols, strict, inClassBody));
        const std::string kw = m[2].str();
        bool isLoop = (kw == "while" || kw == "until");
        bool negated = (kw == "unless" || kw == "until");
        std::string wrappedCond = negated ? "!(" + cond + ")" : cond;
        const std::string opener = isLoop ? "while (" : "if (";
        if (!stmt.empty())
          return opener + wrappedCond + ") { " + stmt + " }";
      }
    }
    ...
    // Ruby's `next` skips to the next iteration. How that is spelled
    // depends on what the enclosing block became: inside a native
    // for-in loop it is `continue`, but inside a block that stayed a
    // closure the "iteration" is one call of that closure, so ending it
    // early is a `return`. (`continue` there would be loop control with
    // no loop, which the compiler cannot express.)
    if (strict && (code == "next" || startsWith(code, "next "))) {
      std::string keyword = "return";
      for (auto it = stack.rbegin(); it != stack.rend(); ++it) {
        if (it->kind == RBFrameKind::Loop) {
          keyword = "continue";
          break;
        }
        if (it->kind == RBFrameKind::ClosureDo ||
            it->kind == RBFrameKind::ClosureLambda ||
            it->kind == RBFrameKind::Def)
          break; // a closure boundary — `return` it is
      }
      code = keyword + code.substr(4);
    }
    ...
    // doesn't apply; build the array explicitly instead.
    {
      ...
      static const std::regex arrayNewBlockRe(
          "Array\\.new\\(([^()]*)\\)\\s*\\{\\s*(?:\\|([^|]*)\\|)?\\s*([^{}]*?)"
          "\\s*\\}");
      code = std::regex_replace(code, arrayNewBlockRe,
                                "range(0, $1).map(fn($2) { return $3 })");
    }

    RubyBlockCall block;
    if (rbTryInlineBraceBlock(code, block, symbols)) {
      std::string prefix = rbNormalizeAtoms(
          rbConvertRanges(block.prefix, strict), symbols, strict, inClassBody);
    ...
    code = rbNormalizeAtoms(code, symbols, strict, inClassBody);
  ...
  for (size_t li = 0; li < rawLines.size(); li++) {
    ...
    if (strict) {
      ...
      for (size_t i = 0; i < code.length(); i++) {
        if (code[i] == '\'' && !inDQuote && (i == 0 || code[i - 1] != '\\'))
          inSQuote = !inSQuote;
        else if (code[i] == '"' && !inSQuote && (i == 0 || code[i - 1] != '\\'))
          inDQuote = !inDQuote;
        else if (code[i] == '#' && !inSQuote && !inDQuote) {
          code = trimCopy(code.substr(0, i));
          break;
        }
      }
      if (code.empty()) {
    ...
    // ── Multiple assignment (whole-statement only) ──────────────────
    {
      ...
      if (rbTryMultiAssign(code, expanded)) {
        for (size_t k = 0; k < expanded.size(); k++) {
          std::string ln = rbConvertInterpolation(
              rbConvertRanges(expanded[k], strict), symbols);
          ln = rbNormalizeAtoms(ln, symbols, strict, inClassBody);
    ...
    if (startsWith(code, "elsif ") && stack.size() > 1 &&
      ...
      f.assignTarget =
          carriedTarget; // an if-expression keeps its target across the chain
      stack.push_back(f);
      continue;
    }
    ...
    if (strict) {
      static const std::regex assignIfRe("^(.+?)\\s*=\\s*(if|unless)\\s+(.+)$");
      std::smatch m;
      if (std::regex_match(code, m, assignIfRe) &&
          rbFindTopLevelAssign(code) != std::string::npos &&
          rbHasMatchingEnd(rawLines, li)) {
        std::string target =
            rbNormalizeAtoms(rbConvertRanges(trimCopy(m[1].str()), strict),
                             symbols, strict, inClassBody);
        std::string cond = rbNormalizeAtoms(
            rbConvertInterpolation(
                rbConvertRanges(trimCopy(m[3].str()), strict), symbols),
    ...
    if (startsWith(code, "unless ") &&
      ...
      outLines.push_back(indentation + "if (!(" + cond + ")) {");
      stack.push_back(
          RBFrame{RBFrameKind::Branch, nextChainGroup++, RBTailInfo{}});
      continue;
    }
    ...
    if (stack.size() > 1) {
      static const std::regex rescueRe(
          "^rescue(\\s+[A-Za-z_][A-Za-z0-9_:]*)?\\s*(=>\\s*(\\w+))?\\s*$");
      std::smatch m;
    ...
    }
    {
      ...
      if (std::regex_match(code, m, whenRe) && stack.size() > 1 &&
        ...
        std::string valuesText =
            rbNormalizeAtoms(rbConvertInterpolation(
                                 rbConvertRanges(m[1].str(), strict), symbols),
                             symbols, strict, inClassBody);
        ...
        if (m[3].matched) {
          ...
          stack.push_back(RBFrame{
              RBFrameKind::Branch, gid,
              RBTailInfo{RBTailKind::Statement, (int)outLines.size() - 1, -1}});
        } else {
    ...
    }
    {
      ...
      static const std::regex includeMarkerRe(
    ...
    if (startsWith(code, "def ") &&
      ...
      // assignment to a parameter is a reassignment (never a `let`).
      {
        ...
        if (lp != std::string::npos && rp != std::string::npos && rp > lp)
          for (auto &p :
               rbSplitTopLevel(signature.substr(lp + 1, rp - lp - 1), ',')) {
    ...
    if (rbTryTrailingDoOpener(code, prefix, params) && !prefix.empty() &&
      ...
      // `f = lambda do |x| ... end` / `proc do |x|` / `Proc.new do |x|`
      // — a bare closure literal, not a call taking a block. Emitting
      // it as one would call an undefined `lambda`. Drop the keyword
      // and let the ClosureDo frame close it as `fn(x) { ... }`.
      if (strict) {
        static const std::regex lambdaDoRe(
            "^(.*?)\\b(?:lambda|proc|Proc\\.new)\\s*$");
        std::smatch lm;
        if (std::regex_match(prefix, lm, lambdaDoRe)) {
          std::string head = lm[1].str(); // e.g. "visit = " or ""
          outLines.push_back(indentation + head + "fn(" + params + ") {");
          stack.push_back(
              RBFrame{RBFrameKind::ClosureLambda, -1, RBTailInfo{}});
          continue;
        }
      }
      std::string convertedPrefix = rbNormalizeAtoms(
          rbConvertRanges(prefix, strict), symbols, strict, inClassBody);

      // `coll.each do |x| ... break/next ... end` — loop control cannot
      // cross into a closure, so emit a native for-in loop instead of a
      // block argument. `break`/`next` then compile to real loop
      // control (and `return` returns from the enclosing method, which
      // is also what Ruby means here).
      if (strict && rbBlockBodyUsesLoopControl(rawLines, li)) {
        static const std::regex eachCallRe("^(.*)\\.each$");
        std::smatch em;
        auto names = rbSplitTopLevel(params, ',');
        if (std::regex_match(convertedPrefix, em, eachCallRe) &&
            names.size() == 1 && !trimCopy(names[0]).empty()) {
          outLines.push_back(indentation + "for " + trimCopy(names[0]) +
                             " in " + em[1].str() + " {");
          stack.push_back(RBFrame{RBFrameKind::Loop, -1, RBTailInfo{}});
          continue;
        }
  ...
  // Ruby happily uses `string`, `int`, `float` etc. as variable/parameter names
  // but Quantum's lexer maps them to TYPE_STRING, TYPE_INT etc., breaking the
  // parse.
  if (strict) {
    static const std::vector<std::pair<std::regex, std::string>> typeRenames = {
        {std::regex("\\bstring\\b"), "_string_"},
        {std::regex("\\bint\\b"), "_int_"},
        {std::regex("\\bfloat\\b"), "_float_"},
        {std::regex("\\bdouble\\b"), "_double_"},
        {std::regex("\\bchar\\b"), "_char_"},
        {std::regex("\\blong\\b"), "_long_"},
        {std::regex("\\bshort\\b"), "_short_"},
    };
  ...
  if (getenv("DUMP_RUBY")) {
    std::cerr << "--- TRANSLATED RUBY ---" << std::endl;
