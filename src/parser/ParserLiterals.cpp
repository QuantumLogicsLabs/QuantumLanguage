...
ASTNodePtr Parser::parsePrimary() {
  ...
  if (tok.type == TokenType::IDENTIFIER && tok.value == "lambda") {
    ...
    if (sawColon) {
      ...
      auto retStmt =
          std::make_unique<ASTNode>(ReturnStmt{std::move(bodyExpr)}, eln);
      BlockStmt block;
  ...
  if (tok.type == TokenType::NEW) {
    ...
    if (check(TokenType::IDENTIFIER))
      ...
      throw ParseError("Expected type name after 'new'", current().line,
                       current().col);

    // new char *[n] — pointer-to-type array: skip any STAR tokens after the
    // type name e.g. "new char *[size]" → treat as array of pointers (same as
    // new char[size] at runtime)
    while (check(TokenType::STAR))
      consume(); // discard pointer qualifier

    // new float[n] — array allocation: allocate array of n default-zero
    // elements
    ...
    // new int(100) / new ClassName(args) — heap allocation, always returns a
    // pointer
  ...
  if (tok.type == TokenType::SUPER) {
    ...
    if (check(TokenType::DOT)) {
      consume(); // eat '.'
      auto method =
          expect(TokenType::IDENTIFIER, "Expected method name after 'super.'")
              .value;
      return std::make_unique<ASTNode>(SuperExpr{method}, ln);
    }
    // bare super — used as super(args) which becomes a CallExpr wrapping
    // SuperExpr
  ...
  if (tok.type == TokenType::LBRACKET) {
    ...
    if (bdepth == 0 && la < tokens.size() &&
        tokens[la].type == TokenType::LPAREN) {
      size_t la2 = la + 1;
      int pdepth = 1;
      ...
      while (la2 < tokens.size() && tokens[la2].type == TokenType::NEWLINE)
        la2++;
      if (pdepth == 0 && la2 < tokens.size() &&
          tokens[la2].type == TokenType::LBRACE) {
        int ln2 = tok.line;
        while (pos < la)
  ...
  if (tok.type == TokenType::RETURN) {
    consume();                // eat 'return'
    return parseReturnStmt(); // parse expression return value natively inline
  }
  ...
  if (tok.type == TokenType::LPAREN) {
    ...
    // First, speculatively check for (TypeName *) or (TypeName*) C-style
    // pointer casts e.g. (Node *)(arena + arena_used), (MyClass *)ptr
    {
      size_t p = pos;
      // Skip optional 'struct'/'const'
      if (p < tokens.size() && tokens[p].type == TokenType::IDENTIFIER &&
          (tokens[p].value == "struct" || tokens[p].value == "const" ||
           tokens[p].value == "unsigned"))
        p++;
      if (p < tokens.size() && tokens[p].type == TokenType::IDENTIFIER) {
        size_t p2 = p + 1;
        ...
        if (p2 < tokens.size() && tokens[p2].type == TokenType::RPAREN) {
          ...
          while (after < tokens.size() &&
                 tokens[after].type == TokenType::NEWLINE)
            after++;
          ...
          // Distinguish from plain grouped expr: (Node) vs (Node *) — only fire
          // if star present OR known type
          ...
          if (hasStar && valueFollows) {
            ...
            // If it's a cast to a struct/class pointer (not a primitive),
            // simulate malloc by creating a new instance
            if (castType != "void" && castType != "char" && castType != "int" &&
                castType != "float" && castType != "double" &&
                castType != "long" && castType != "short" &&
                castType != "unsigned") {
              // Generate ast: ClassName()
              int castLn = tokens[p].line;
              auto typeId =
                  std::make_unique<ASTNode>(Identifier{castType}, castLn);
              std::vector<ASTNodePtr> noArgs;
              return std::make_unique<ASTNode>(
                  CallExpr{std::move(typeId), std::move(noArgs)}, castLn);
            }
            return innerExpr;
          }
        }
    ...
    if (check(TokenType::RPAREN) && expr->is<Identifier>()) {
      ...
      // Known C type names used in casts (all lowercase — distinct from
      // compound literals)
      ...
      if (cCastTypes.count(idName)) {
        // Peek: if after ')' there is a value-producing token (not
        // end/newline/;), this is a cast — eat ')' and parse the cast operand.
        size_t peekPos = pos + 1;
        while (peekPos < tokens.size() &&
    ...
    // Multi-word C-style cast: (long long)x  (unsigned int)x — inner expr is
    // BinaryExpr or similar Detect: inside parens we see TYPE TYPE (two type
    // keywords) — these were lexed as separate identifier/type tokens and
    // combined by the expression parser. Check if expr is a BinaryExpr or if we
    // have a trailing type keyword before ')'. Handle: (long long) — first
    // token was 'long' (identifier), second is 'long' (identifier) The parser
    // sees 'long' as Identifier, then space, then 'long' which is another
    // identifier. Actually 'long long' is parsed as two identifier tokens which
    // can't be a valid expr, so we also handle the case where current() is a
    // C-type keyword before ')'.
    if (isCTypeKeyword(current().type)) {
    ...
    // e.g. Room, Entity, Point). This prevents mis-firing on normal
    // control-flow conditions like while(cond) { } or if(flag) { }.
    if (check(TokenType::RPAREN) && expr->is<Identifier>()) {
      const std::string &idName = expr->as<Identifier>().name;
      bool looksLikeType =
          !idName.empty() && std::isupper((unsigned char)idName[0]);
      ...
      if (looksLikeType) {
        ...
        while (peekPos < tokens.size() &&
          ...
          ++peekPos;
        if (peekPos < tokens.size() &&
            tokens[peekPos].type == TokenType::LBRACE) {
    ...
    if (check(TokenType::FOR)) {
      ...
      auto readVar = [&]() {
        if (check(TokenType::IDENTIFIER))
          ...
          vars.push_back(
              expect(TokenType::IDENTIFIER, "Expected variable").value);
      };
      readVar();
      while (match(TokenType::COMMA))
        readVar();
      if (!match(TokenType::IN) && !match(TokenType::OF))
        throw ParseError("Expected 'in' in generator expression",
  ...
  // C-type keywords used as variable names (e.g. "string = 'hello'", "double
  // = 3.14")
  ...
  // Built-in keyword tokens that can appear as callable expressions in rhs
  // context. e.g.  x = input("prompt")   result = print(...)   val = len(arr)
  ...
  if (check(TokenType::FOR)) {
    ...
    auto readVar = [&]() {
      if (check(TokenType::IDENTIFIER))
        ...
        vars.push_back(
            expect(TokenType::IDENTIFIER, "Expected variable in comprehension")
                .value);
    };
    readVar();
    while (match(TokenType::COMMA))
    ...
    if (!match(TokenType::IN) && !match(TokenType::OF))
      throw ParseError("Expected 'in' in list comprehension", current().line,
                       current().col);
...
ASTNodePtr Parser::parseDictLiteral() {
  ...
  while (!check(TokenType::RBRACE) && !atEnd()) {
    ...
    if (check(TokenType::IDENTIFIER) || isCTypeKeyword(current().type) ||
      ...
      // Peek ahead — if next token after this is COLON, treat as bare string
      // key
      size_t la = pos + 1;
      while (la < tokens.size() && tokens[la].type == TokenType::NEWLINE)
        la++;
      ...
      } else if (la < tokens.size() && (tokens[la].type == TokenType::COMMA ||
                                        tokens[la].type == TokenType::RBRACE)) {
    ...
    ASTNodePtr val;
    if (isShorthand) {
      val = std::make_unique<ASTNode>(
          Identifier{key->as<StringLiteral>().value}, ln);
    } else {
...
std::vector<ASTNodePtr> Parser::parseArgList() {
  ...
  while (!check(TokenType::RPAREN) && !atEnd()) {
    ...
    if (check(TokenType::POWER)) // ** is POWER
    {
      ...
      args.push_back(
          std::make_unique<ASTNode>(UnaryExpr{"**", std::move(expr)}, pLn));
      skipNewlines();
      if (!match(TokenType::COMMA))
        break;
      skipNewlines();
...
Parser::parseParamList(std::vector<bool> *outIsRef,
  ...
  while (!check(TokenType::RPAREN) && !atEnd()) {
    ...
    if (check(TokenType::IDENTIFIER)) {
      params.push_back(prefix + consume().value);
      if (outIsRef)
    ...
    if (check(TokenType::COLON)) {
      ...
      if (outParamTypes) {
        while (outParamTypes->size() < params.size() - 1)
      ...
      while (!atEnd()) {
        if (check(TokenType::LBRACKET) || check(TokenType::LPAREN))
          ...
          depth--;
                   ...
                   (check(TokenType::COMMA) || check(TokenType::ASSIGN)))
          break;
        consume();
      }
...
ASTNodePtr Parser::parseCTypeVarDecl(const std::string &typeHint) {
  ...
  // Check for optional '(' for function pointers like void (*fp)() or void
  // (Class::*fp)()
  bool hasParen = false;
    ...
    if (la < tokens.size() && tokens[la].type == TokenType::COLON) {
      ...
      if (la2 < tokens.size() && tokens[la2].type == TokenType::COLON) {
        std::string className =
            consume().value; // eat ClassName (discard for naming)
        consume();           // colon
        consume();           // colon
  ...
  Token nameToken =
      check(TokenType::IDENTIFIER) ? consume()
      : (check(TokenType::INPUT) || check(TokenType::PRINT))
          ? consume()
          : expect(TokenType::IDENTIFIER, "Expected variable name after type");

  if (hasParen) {
    expect(TokenType::RPAREN,
           "Expected ')' inside function pointer declaration");
  ...
  if (match(TokenType::ASSIGN)) {
    ...
    if (!arrayDims.empty() && init && init->is<ArrayLiteral>()) {
      ...
      zeroFillIdiom =
          els.empty() || (els.size() == 1 && els[0]->is<NumberLiteral>() &&
  ...
  // would swallow the separator in  for (int i = 0; i < n; i++)  and break the
  // parse. Only skip bare newlines that are purely cosmetic whitespace.
  while (check(TokenType::NEWLINE))
    consume();
  return node;
}