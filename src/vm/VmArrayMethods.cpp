...
QuantumValue VM::callArrayMethod(std::shared_ptr<Array> arr,
  ...
  if (m == "includes" || m == "contains" || m == "include" || m == "member") {
    if (args.empty())
  ...
  if (m == "splice") {
    ...
    int deleteCount =
        args.size() > 1 ? (int)args[1].asNumber() : (int)arr->size() - idx;
    if (idx < 0)
      idx = std::max(0, (int)arr->size() + idx);
    idx = std::min(idx, (int)arr->size());
    deleteCount = std::max(0, std::min(deleteCount, (int)arr->size() - idx));
    auto removed = std::make_shared<Array>(arr->begin() + idx,
                                           arr->begin() + idx + deleteCount);
  ...
  if (m == "take") {
    ...
    return QuantumValue(
        std::make_shared<Array>(arr->begin(), arr->begin() + n));
  }
  if (m == "extend") {
  ...
  // Helper: call a QuantumValue (closure, native, or bound method) with given
  // args
  auto callFn = [&](QuantumValue fn,
                    std::vector<QuantumValue> fnArgs) -> QuantumValue {
    if (fn.isNative())
  ...
  if (m == "sort_by" || m == "min_by" || m == "max_by") {
    ...
    for (auto &kv : keyed)
      if (m == "min_by" ? keyLess(kv.first, best.first)
                        : keyLess(best.first, kv.first))
        best = kv;
    return best.second;
  }
  ...
  if (m == "each_with_index" || m == "with_index") {
    if (args.empty()) {
  ...
  if (m == "each_index") {
    if (args.empty()) {
      auto indices = std::make_shared<Array>();
      for (size_t i = 0; i < arr->size(); ++i)
        indices->push_back(QuantumValue((double)i));
      return QuantumValue(indices);
    }
    for (size_t i = 0; i < arr->size(); ++i)
      callFn(args[0], {QuantumValue((double)i)});
    return QuantumValue(arr);
  }
  if (m == "forEach" || m == "each") {
    if (args.empty())
